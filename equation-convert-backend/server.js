...
function runTextContent(inner) {
  // The editor's content-loader only recognizes $...$/$$...$$ LaTeX and [IMG:n]
  // tokens — it does NOT parse arbitrary HTML, so <sup>/<sub> tags would show up
  // as literal visible text. Its own x²/x₂ toolbar buttons produce superscripts
  // via LaTeX "^{...}"/"_{...}" rendered through KaTeX, so match that exactly.
  ...
  if (out && isSuper) out = `$^{${out}}$`;
  else if (out && isSub) out = `$_{${out}}$`;
  return out;
}
...
// ── Claude API proxy ──────────────────────────────────────────────────────
// examsetu-v19.html's PDF-import AI parser (parseQuestionsWithAI) was calling
// https://api.anthropic.com/v1/messages DIRECTLY from the browser with no API
// key at all. This can never work: (a) Anthropic's API requires an x-api-key
// header, and (b) it doesn't send CORS headers permitting browser calls from
// arbitrary origins anyway, so the browser blocks the response before any JS
// runs — both failure modes show up identically as a generic "Failed to
// fetch" with no further detail, which is why it looked mysterious. Fix: the
// browser calls THIS route instead; this server holds the real API key (as an
// env var, never shipped to the browser) and forwards the request.
const ANTHROPIC_API_KEY = process.env.ANTHROPIC_API_KEY || '';
app.use(express.json({ limit: '10mb' }));
app.post('/api/claude-proxy', async (req, res) => {
  if (!ANTHROPIC_API_KEY) {
    return res.status(500).json({ error: 'ANTHROPIC_API_KEY is not set on the server (Render → Environment tab)' });
  }
  try {
    ...
    res.status(502).json({ error: 'Could not reach Claude API: ' + e.message });
  }
});
